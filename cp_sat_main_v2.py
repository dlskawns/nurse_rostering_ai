"""
main_v2.py와 동일한 품질의 CP-SAT + LNS 근무표 생성 엔진

이 엔진은 다음을 포함합니다:
- CP-SAT + LNS 2단계 최적화 (main_v2.py 방식)
- 상세 메트릭 계산 및 분석
- 향상된 만족도 분석
...
main_v2.py의 모든 기능을 DB 호환 방식으로 구현
"""
...
class CPSATMainV2Engine:
    """main_v2.py와 동일한 품질의 CP-SAT + LNS 근무표 생성 엔진"""
    
    def __init__(self):
        self.logger_prefix = "[CP-SAT-Main-V2]"
    ...
    def parse_preferences_from_db(self, prefs_data: List[dict]) -> Tuple[Dict, Dict, Dict]:
        """
        DB에서 가져온 선호도 데이터를 main_v2.py 형식으로 변환
        ...
        for pref in prefs_data:
            ...
            # 휴무 요청 파싱 (문자열 ID 그대로 사용)
            if 'off' in data and data['off']:
                # roster_system은 문자열 ID를 사용하므로 그대로 유지
                off_requests[nurse_id] = {}
                for date_str in data['off']:
                    try:
                        day = int(date_str)
                        # roster_system 형식에 맞게 {날짜(str): delta_weight(float)} 변환
                        off_requests[nurse_id][str(day)] = 5.0  # 기본 델타 가중치
                ...
                for pref_item in data['preference']:
                    if isinstance(pref_item, dict):
                        ...
                        if pref_item.get('shift') == 'OFF':
                            ...
                            weight = pref_item.get('weight', 5.0)  # 델타 가중치
                            if date:
    ...
    def generate_roster(
        ...
        """
        main_v2.py와 동일한 품질의 CP-SAT + LNS 근무표 생성
        ...
        print(f"\n{'='*60}")
        print(f"{self.logger_prefix} main_v2.py 품질 근무표 생성 시작")
        ...
        # 7. 선호 근무 유형 적용 (main_v2.py 핵심 기능)
        ...
        # 9. CP-SAT으로 1차 최적화 (main_v2.py 방식: 30초)
        cp_sat_time_limit = min(30, time_limit_seconds // 2)  # CP-SAT에 절반 시간 할당
        with Timer("CP-SAT 1차 최적화"):
            print(f"{self.logger_prefix} CP-SAT 최적화 시작 (시간 제한: {cp_sat_time_limit}초)...")
            success = roster_system.optimize_roster_with_cp_sat(time_limit_seconds=cp_sat_time_limit)
            
            if not success:
                print(f"{self.logger_prefix} CP-SAT 최적화 실패!")
                return {}
        
        # 10. LNS로 2차 최적화 (main_v2.py 방식: 5회 반복, 10초씩)
        with Timer("LNS 2차 최적화"):
            print(f"{self.logger_prefix} LNS 추가 최적화 시작...")
            lns_time_per_iteration = min(10, (time_limit_seconds - cp_sat_time_limit) // 5)
            roster_system.optimize_with_lns(
                max_iterations=5,  # main_v2.py와 동일
                time_limit_per_iteration=lns_time_per_iteration
            )
        
        # 11. 최적화 결과 분석 및 출력 (main_v2.py 수준의 상세 분석)
        with Timer("최적화 결과 분석"):
            self._analyze_optimization_results(roster_system)
        
        # 12. 상세 메트릭 계산 (main_v2.py 핵심 기능)
        with Timer("상세 메트릭 계산"):
            try:
                metrics = roster_system.calculate_detailed_metrics()
                self._print_detailed_metrics_summary(metrics)
                
                # 메트릭을 로그로 저장 (main_v2.py 방식)
                print(f"{self.logger_prefix} 상세 메트릭이 계산되었습니다.")
            except Exception as e:
                print(f"{self.logger_prefix} 상세 메트릭 계산 중 오류: {e}")
        
        # 13. 만족도 분석 (main_v2.py 고유 기능)
        with Timer("만족도 분석"):
            self._analyze_satisfaction_rates(roster_system)
        
        # 14. 결과 변환
        with Timer("결과 변환"):
            result = self._convert_result_to_db_format(roster_system, nurses)
        
        print(f"\n{self.logger_prefix} main_v2.py 품질 근무표 생성 완료!")
        print(f"{'='*60}")
        
        return result
    
    def _analyze_optimization_results(self, roster_system: RosterSystem):
        """최적화 결과를 main_v2.py 수준으로 상세 분석"""
    ...
    def _analyze_satisfaction_rates(self, roster_system: RosterSystem):
        """만족도 분석 (main_v2.py 고유 기능)"""
        print(f"\n{self.logger_prefix} 만족도 분석:")
...
# 전역 엔진 인스턴스
cp_sat_main_v2_engine = CPSATMainV2Engine()


def generate_roster_cp_sat_main_v2(nurses_data, prefs_data, config_data, year, month, time_limit_seconds=60):
    """
    main_v2.py와 동일한 품질의 CP-SAT + LNS 근무표 생성 (기존 인터페이스 호환)
    ...
    Args:
        ...
        year: 근무표 년도
        month: 근무표 월
        time_limit_seconds: 전체 최적화 시간 제한
        
    Returns:
        Dict[nurse_id, List[shift]]: 간호사별 일일 근무 배정
    """
    return cp_sat_main_v2_engine.generate_roster(
